...
window.onload = function () {
    var nr =(localStorage.getItem('numaProduse'));
    let i;
    if (typeof(Storage) !== "undefined" && nr>0) {
        nrProduse = Number(nr);
        for (i = 1; i <= nr; i++) {
            // Retrieve
            document.getElementById("list").insertAdjacentHTML('beforeen', localStorage.getItem(i));
        }
    }
    else{
        nrProduse = 0;
...
document,getElementById('list').addEventListener('dblclick', function(event){
    const element = event.target;
    let strorageKey = Number(element.id) +1;
    nrProduse = nrProduse -1;
